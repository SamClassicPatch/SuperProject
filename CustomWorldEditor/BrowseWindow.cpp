...
HGLOBAL CreateHDrop( const CTFileName &fnToDrag, BOOL bAddAppPath/*=TRUE*/)
{
  ...
  if( bAddAppPath)
  {
    fnFullToDrag = IDir::AppPath() + fnToDrag;
  }
  else
  {
...
void CBrowseWindow::OpenDirectory( CVirtualTreeNode *pVTNDir)
{
  ...
  #ifdef UNICODE
    swprintf( achrOpenedDirectoryMessage, _T("Opened directory: \"%s\""), CString(theApp.m_strOpenedVTDirectory));
  #else
    sprintf( achrOpenedDirectoryMessage, _T("Opened directory: \"%s\""), CString(theApp.m_strOpenedVTDirectory));
  #endif
...
void CBrowseWindow::OnLButtonDblClk(UINT nFlags, CPoint point)
{
  ...
  if( fnItem.FileExt() == ".tex")
  {
...
#endif
    {
      ...
      theApp.SetNewActiveTexture(IDir::AppPath() + fnItem);
  ...
  else if( fnItem.FileExt() == ".wld")
  {
    ...
    theApp.m_pDocTemplate->OpenDocumentFile(CString(IDir::AppPath() + fnItem));
  }
}
...
{
  // set it as new primitive's material default texture
  theApp.SetNewActiveTexture(IDir::AppPath() + _fnRightClickedItemFileName);
}
