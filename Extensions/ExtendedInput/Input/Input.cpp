...
void CInputPatch::Construct(void)
{
  ...
  ASSERT(SDL_GAMEPAD_AXIS_COUNT == MAX_AXES_PER_JOYSTICK);
  ASSERT(EIA_CONTROLLER_OFFSET == FIRST_JOYAXIS);
  ASSERT(EIA_MAX_ALL == MAX_OVERALL_AXES);

  ASSERT(SDL_GAMEPAD_BUTTON_COUNT <= MAX_BUTTONS_PER_JOYSTICK); // Less controller buttons than joystick ones
  ...
  if (!SDL_Init(SDL_INIT_GAMEPAD)) {
    CPrintF("SDL_Init(SDL_INIT_GAMEPAD) failed:\n%s\n", SDL_GetError());
  }
