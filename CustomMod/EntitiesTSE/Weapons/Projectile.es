...
void CProjectile_OnPrecache(CDLLEntityClass *pdec, INDEX iUser) 
{
  ...
  case PRT_EARTHMAN_STONE: // [Cecil] Projectile from Revolution
  ...
  case PRT_WATERMAN_SMALL:
    pdec->PrecacheSound(SOUND_FLYING); // [Cecil] 'SOUND_DEMON_FLYING' -> 'SOUND_FLYING'
    pdec->PrecacheModel(MODEL_LARVA_PLASMA);
    pdec->PrecacheTexture(TEXTURE_LARVA_PLASMA);
    pdec->PrecacheModel(MODEL_LARVA_PLASMA_BALL); // [Cecil]
    pdec->PrecacheTexture(TEXTURE_LARVA_PLASMA_BALL); // [Cecil]
    pdec->PrecacheClass(CLASS_BASIC_EFFECT, BET_LIGHT_CANNON);    
    break;
...
functions:
  ...
  void RenderParticles(void) {
    switch (m_prtType) {
      ...
      case PRT_AIRMAN_LARGE: case PRT_AIRMAN_BIG:
        Particles_Windblast(this, m_fStretch / 4.0f, m_fStartTime + 3.0f);
        break;
...
// [Cecil] Player plasma projectiles from Revolution
...
// [Cecil] Old water elemental projectile from Revolution
...
// [Cecil] Water elemental projectiles from Revolution
...
// [Cecil] Earth elemental projectiles from Revolution
...
// [Cecil] Explosion effect identical to CyborgBombExplosion() from Revolution
...
// [Cecil] Air elemental projectiles from Revolution
void WindBlastRev(FLOAT fStretch, FLOAT fSpeed, FLOAT fSpin, FLOAT fDamage) {
  // set appearance
  InitAsEditorModel();
  SetPhysicsFlags(EPF_MODEL_SLIDING);
  SetCollisionFlags(ECF_PROJECTILE_MAGIC);
...
void WindBlastLarge(void) {
  WindBlastRev(3.0f, 25.0f, (FRnd() * 20.0f) - 10.0f, 20.0f);
};

void WindBlastBig(void) {
  WindBlastRev(2.0f, 25.0f, (FRnd() * 20.0f) - 10.0f, 10.0f);
};

void WindBlastSmall(void) {
  WindBlastRev(1.0f, 25.0f, (FRnd() * 20.0f) - 10.0f, 5.0f);
};
...
void ProjectileTouch(CEntityPointer penHit)
{
  ...
  // [Cecil] Or air projectile from Revolution
  } else if (m_prtType == PRT_AIRELEMENTAL_WIND || bAirProjectile) {
  ...
  Main(ELaunchProjectile eLaunch) {
    ...
    switch (m_prtType)
    {
      ...
      default: {
        ...
        ASSERTALWAYS("Unknown projectile type");
      }
    }
