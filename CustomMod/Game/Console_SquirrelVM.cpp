...
#include <CoreLib/Scripting/ScriptVM.h>
...
static void ScriptTest(CTString str) {
  str = "_Script.nut";

  static sq::ScriptVM vm(sq::SQVM_STDMATH | sq::SQVM_STDSTRING | sq::SQVM_TIMER);
  static INDEX ctStackTop = 0;
  ...
  if (sq_getvmstate(vm) != SQ_VMSTATE_SUSPENDED) {
    ctStackTop = sq_gettop(vm);

    // Compile script (pushes script as a closure on top)
    //r = vm.CompileScript(str, "Shell");
    r = vm.CompileSource("SeriousSam_ClassicsPatch\\" + str);

  } else {
    bWasSuspended = true;
  }

  //vm.PrintCurrentStack(FALSE, "After compile"); // Print the stack

  if (SQ_SUCCEEDED(r)) {
    // Resume suspended VM
    if (sq_getvmstate(vm) == SQ_VMSTATE_SUSPENDED) {
      r = sq_wakeupvm(vm, FALSE, bExpectReturnValue, TRUE, FALSE);

    } else {
      sq_pushroottable(vm); // Push root table as 'this' for the script

      // Pop root table from the stack and call the closure without removing it afterwards
      // Push the return value on top (or 'null' if doesn't return anything)
      r = sq_call(vm, 1, bExpectReturnValue, TRUE);
    }

    //vm.PrintCurrentStack(FALSE, "After call"); // Print the stack

    if (SQ_SUCCEEDED(r)) {
      // Output return value
      if (bExpectReturnValue) {
        vm.PrintTop();
        CPutString("\n");
        sq_poptop(vm); // Pop return value
      }

      if (sq_getvmstate(vm) != SQ_VMSTATE_SUSPENDED) {
        if (bWasSuspended) {
          sq_poptop(vm); // Pop very initial root table that's somehow still in there after waking up from suspension
        }

        sq_poptop(vm); // Pop script closure
      }
  ...
  vm.PrintCurrentStack(FALSE, "At the end"); // Print the stack

  // Quit if the VM is suspended
  if (sq_getvmstate(vm) == SQ_VMSTATE_SUSPENDED) {
    CPrintF("^cffdf00VM IS SUSPENDED | init: %d\n", ctStackTop);
    return;
  }

  // Restore the stack top
  sq_settop(vm, ctStackTop);
  CPrintF("^cffdf00init/end: %d/%d\n", ctStackTop, sq_gettop(vm));

  // Check amount of unreachable objects
  //return;
  sq_resurrectunreachable(vm); // Push array or null
  SQInteger ctRefs = -1;
  ...
  CPrintF("^cffdf00sq_resurrectunreachable() -> %d (stack ct: %d)\n", ctRefs, sq_gettop(vm));
};
...
static void Key_Return(void)
{
  ...
  // [Cecil] Run Squirrel script
  if (strEditingLine.RemovePrefix("sq>")) {
    strEditingLine.TrimSpacesLeft();
    CPrintF("^cffffffsq> %s\n", strEditingLine);

    ScriptTest(strEditingLine);
