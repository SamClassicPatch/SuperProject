...
void CWndDisplayTexture::OnPaint()
{
  ...
  if (m_pDrawPort != NULL) {
    ...
    if (m_bDrawLine) {
      m_pDrawPort->DrawLine(m_pixLineStartU, m_pixLineStartV, m_pixLineStopU, m_pixLineStopV, C_WHITE | CT_OPAQUE, 0xCCCCCCCC);
...
void CWndDisplayTexture::OnLButtonDown(UINT nFlags, CPoint point)
{
  ...
  if (pixU < 0 || pixU > m_pixWinWidth) {
    return;
  }

  if (pixV < 0 || pixV > m_pixWinHeight) {
    return;
  }
